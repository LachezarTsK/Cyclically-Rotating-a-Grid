...
public:
    vector<vector<int>> rotateGrid(vector<vector<int>>& inputMatrix, int cyclicRotations) {
        int rows = inputMatrix.size();
        int columns = inputMatrix[0].size();

        this->cyclicRotations = cyclicRotations;
        this->rotatedMatrix.resize(rows, vector<int>(columns));

        int limit = min(rows / 2, columns / 2);
        int startSource = 0;
        int shrinkSide = 0;
...
private:
    void rotateLayer(int startSourceInLayer, int rowsInLayer, int columnsInLayer, span<const vector<int>> inputMatrix) {
    ...
    void placeRotatedAtStartForNextLayer(int startSourceInLayer, int rotationsInLayer, int rowsInLayer, int columnsInLayer) {
    ...
    void updatePosition(Layer& layer, int startSourceInLayer, int rowsInLayer, int columnsInLayer) {
